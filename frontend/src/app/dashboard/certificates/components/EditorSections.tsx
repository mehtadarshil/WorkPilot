...
export function BoardsListSection() {
  ...
  const base = `/dashboard/certificates/${certificate.id}/boards`;
        ...
        {document.boards.map((board) => (
          <li key={board.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <Link href={`${base}/${board.id}`} className="text-base font-bold text-[#0d9488] hover:underline">
                  {board.name}
                </Link>
    ...
    <div className="mx-auto max-w-3xl">
      <SectionCard title="Appendix notes & photos">
        <TextAreaField
          ...
          onChange={(v) => setDocument((d) => ({ ...d, appendix: { content: v } }))}
          rows={12}
