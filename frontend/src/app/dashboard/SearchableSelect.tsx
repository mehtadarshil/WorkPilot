...
type Props = {
  ...
  /** When value is empty or not in options */
...
export default function SearchableSelect({
  ...
  const buttonLabel = selected ? selected.label : emptyButtonLabel;
                ...
                filtered.map((o) => (
                  <button
                    ...
                    className={`w-full min-w-0 px-3 py-2 text-left text-sm transition ${
                      o.value === value ? 'bg-[#14B8A6]/10 font-medium text-[#14B8A6]' : 'text-slate-900'
                    } hover:bg-slate-50`}
                    onClick={() => {
                      onChange(o.value);
                      setOpen(false);
                  ...
                  >
                    ...
                    {o.hint ? <span className="block truncate text-[11px] text-slate-400">{o.hint}</span> : null}
                  </button>
                ))
              )}
            </div>
