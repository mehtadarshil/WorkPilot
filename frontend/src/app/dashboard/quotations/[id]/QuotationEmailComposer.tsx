...
export default function QuotationEmailComposer({ open, onClose, quotationId, onSent }: Props) {
  ...
  const loadDraft = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getJson<ComposeDraft>(`/quotations/${quotationId}/email-compose`, token);
      ...
      if (scheduleAfterSend) {
        try {
          await postJson(
            ...
            {
              type: 'note',
              text: `Follow-up suggested after emailing quotation. Subject: ${subject.trim()}`,
            },
            token,
          );
        } catch {
