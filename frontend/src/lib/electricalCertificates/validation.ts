...
import { isNaDescription } from './circuitGridUtils';
...
export function validateElectricalCertificate(doc: ElectricalCertificateDocument): ValidationIssue[] {
  if (
    ...
    doc.typeSlug === 'eic_18e_a3' ||
    doc.typeSlug === 'mwc_18e_a3'
  ) {
    return [];
  }
  ...
  for (const item of INSPECTION_SCHEDULE_ITEMS) {
    ...
    if (!outcome) sectionIncomplete[item.section] = (sectionIncomplete[item.section] ?? 0) + 1;
  }
