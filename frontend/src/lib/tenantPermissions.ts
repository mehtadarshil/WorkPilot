...
export const PERMISSION_UI_GROUPS: readonly {
...
}[] = [
  {
    ...
    keys: ['customers', 'jobs', 'quotations', 'invoices', 'invoice_send', 'scheduling'],
  },
  {
    id: 'compliance',
  ...
  },
  {
    id: 'docu',
    title: 'Docu Center',
    description: 'Company reference documents and guides organised in folders.',
    keys: ['docu_center', 'docu_center_manage'],
  },
  {
    id: 'admin',
    title: 'Team & settings',
...
export const PERMISSION_LABELS: Record<TenantPermissionKey, string> = {
  ...
  todos: 'Todos & tasks',
  docu_center: 'Docu Center (view)',
  docu_center_manage: 'Docu Center (manage)',
...
export const PERMISSION_HINTS: Record<TenantPermissionKey, string> = {
  ...
  invoice_send: 'Email issued invoices to clients (field officers need this to send; without it they can only create drafts).',
  scheduling: 'Calendar visits, leave overlays, and dispatch (also drives field visit list on the app when linked).',
  ...
  docu_center: 'Open Docu Center and browse folders your role is allowed to see.',
  docu_center_manage: 'Create folders, upload/delete files, and set who can access each folder.',
