...
Future<void> showQuotationOfficialSendSheet(
  BuildContext context, {
  required int quotationId,
  required VoidCallback onSent,
}) async {
  final repo = Get.find<QuotationsRepository>();
  Map<String, dynamic>? draft;
  try {
    draft = await repo.getEmailComposeDraft(quotationId);
  } catch (_) {}
  ...
  final canSend = draft?['can_send'] != false;
                              ...
                              if (scheduleFollowUp) {
                                try {
                                  await repo.postCommunication(
                                    ...
                                    {
                                      'type': 'note',
                                      'text': 'Follow-up suggested after emailing quotation. Subject: $subject',
                                    },
                                  );
                                } catch (_) {}
                              }
