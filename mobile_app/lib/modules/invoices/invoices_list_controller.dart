...
class InvoicesListController extends GetxController {
  InvoicesListController({InvoicesRepository? repo}) : _repo = repo ?? Get.find<InvoicesRepository>();

  final InvoicesRepository _repo;

  final RxList<Map<String, dynamic>> items = <Map<String, dynamic>>[].obs;
  final RxMap<String, int> stateCounts = <String, int>{}.obs;
  final RxMap<String, double> stateTotals = <String, double>{}.obs;
  final RxDouble overallOutstanding = 0.0.obs;
      ...
      if (append) {
        ...
        if (stats is Map) {
          ...
          for (final e in stats.entries) {
            ...
            final v = e.value;
            if (v is Map) {
              c[key] = (v['count'] as num?)?.toInt() ?? 0;
              t[key] = (v['total_amount'] as num?)?.toDouble() ?? double.tryParse('${v['total_amount']}') ?? 0;
            }
          }
          stateCounts.assignAll(c);
          stateTotals.assignAll(t);
        } else {
          stateCounts.clear();
          stateTotals.clear();
        }
        overallOutstanding.value = (raw['overallOutstanding'] as num?)?.toDouble() ?? 0;
      }
